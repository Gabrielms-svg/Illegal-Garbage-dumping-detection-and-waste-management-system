...
EVIDENCE_DIR = "evidence"        # directory with dumping events
...
event_id = 0
processed_videos = set()  # keeps track of already processed evidence
...
while True:
    ...
    for root, dirs, files in os.walk(EVIDENCE_DIR):
        for f in files:
            if f.endswith(".mp4"):
                ...
                # Process this video
                saved_tracks.clear()  # reset per video
                while True:
                    ret, frame = cap.read()
                    if not ret:
                        break
                    ...
                    for box in results[0].boxes:
                        ...
                        print(f"[SAVED] Vehicle {event_id} from {f}")
    ...
    time.sleep(3)  # wait 3 sec before scanning for new evidence
